System.register(["../components", "angular", "../textfield/textfield"], function($__export) {
  "use strict";
  ...
  return {
    ...
    execute: function() {
      FIELD_ID_COUNTER = 1;
      materialComponents.directive('materialSearchfield', ['$parse', function($parse) {
        return {
          restrict: 'EA',
          scope: {
            fid: '@?fieldId',
          ...
          require: '?ngModel',
          template: ['<div class="{{getIconCls(\'search\')}}"></div>', '<div class="{{getIconCls(\'clear\')}}" ng-click="clearField($event)"></div>', '<input ng-model="value" id="{{fid}}" ng-disabled="isDisabled()" type="text" placeholder="{{placeholder}}"></input>'].join(''),
